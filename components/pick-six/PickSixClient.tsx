...
function PickSixClientInner({ profile, existingEntry, userId, initialStep, entryCount = 0 }: Props) {
  ...
  async function handleSignupAndSave(e: React.FormEvent) {
    ...
    // Always file picks into the General league.
    ...
    // If they came from /sweepstake?then=pick-six, also save the sweepstake entry
    // alongside the pick-six entry. Single signup, two entries.
    if (sweepstakeIntent) {
      ...
      if (sweepErr) {
        // Pick Six entry is saved; let the user know the sweepstake didn't persist
        // but keep them moving forward to payment.
